...
- **같은 사진 세트** — 기본 3장 + 옵션 1장, 장당 ~4.2MB로 통일
...
| 구간 (기본 3 + 옵션 1, ~4.2MB/4장) | 순차 (median) | 병렬 (median) | 단축   |
| ---------------------------------- | ------------- | ------------- | ------ |
...
병렬 업로드(측정 1)의 절대 단축은 작아요. Fast 4G 100초 중 2초, Wi-Fi 2.5초 중 0.5초 — 2~21%. 글 앞에서 모델로 짐작했던 그대로 **전송이 지배적인 환경에선 병렬의 이득이 거의 사라진다**는 게 숫자로 찍혔습니다. 같은 4장(~4.2MB)인데도 Wi-Fi 쪽이 상대적 단축이 더 큰 게 그 증거고요 — Fast 4G는 좁은 업로드 대역폭이 시간을 다 가져가서 핸드셰이크를 겹쳐도 깎을 여지가 거의 없어요. 4G 사용자의 100초를 진짜로 깎고 싶다면 **사진 자체를 줄여야** 합니다. 클라이언트 압축·리사이즈가 다음 자연스러운 단계예요.
...
- `4G의 100초` 이번 작업이 **옮기지 못한 시간**이다. 시간을 옮기는 게 아니라 **없애려면** 업로드 바이트 자체를 줄여야 하고 거기서부터는 다른 이야기다. 그건 다른 글에서 다룰 예정이다.
<!-- TODO : 다른 글을 작성한다면 링크로 추가 할 것.. -->
