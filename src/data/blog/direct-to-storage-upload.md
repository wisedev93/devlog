...
신청 폼의 마지막 단계는 사진 업로드입니다. 기본 사진 1~3장, 옵션 사진 0~1장, 장당 최대 10MB까지 허용했어요. 로컬에서는 잘 됐습니다. 그런데 배포한 뒤 실제 폰에서 사진 몇 장을 골라 제출하니 **요청이 그냥 실패**했어요. 응답 코드는 `413`.
...
처음 구조는 평범했습니다. 신청 폼이 `FormData`에 텍스트 필드와 사진 `File` 들을 한꺼번에 담아 `POST /api/apply` 로 보내고, 서버 함수가 그걸 받아서 검증 → Storage 업로드 → DB insert 를 합니다.
...
export async function POST(req: NextRequest) {
  ...
  // ...검증 후 서버에서 Storage 로 업로드
}
```

코드만 보면 문제가 없어요. 문제는 코드 바깥, **실행 환경**에 있었습니다.

> Vercel 서버리스 함수는 요청 본문이 **4.5MB** 로 제한됩니다. 공식 문서에 명시된 하드 한도이고, 코드로 늘릴 수 있는 값이 아니에요.

우리 폼은 사진을 장당 10MB까지 허용합니다. 기본 3장 + 옵션 1장이면 최대 40MB. 4.5MB의 거의 아홉 배예요. 사용자가 요즘 폰으로 찍은 사진 몇 장만 올려도 한도를 가뿐히 넘깁니다. 그래서 본문이 한도를 넘는 순간 함수 코드는 **실행되기도 전에** 게이트웨이에서 잘리고, 클라이언트는 `413 Content Too Large` 만 받습니다.
...
핵심 아이디어는 이렇습니다. 사진은 어차피 Supabase Storage 로 갑니다. 그렇다면 **굳이 Vercel 함수를 중간에 거칠 이유가 없어요.** 브라우저가 Storage 로 직접 올리면 4.5MB 한도는 우리 함수와 무관한 이야기가 됩니다.

문제는 인가예요. Storage 버킷에는 RLS 가 켜져 있고, 아무 브라우저나 마음대로 쓰게 둘 수는 없습니다. 여기서 쓰는 게 **서명 업로드 URL(signed upload URL)** 입니다.

흐름은 3단계로 쪼개집니다.

1. 브라우저가 서버에 "사진 N장 올릴 건데 자리 좀 잡아줘" 라고 요청 → 서버가 **업로드용 티켓**(경로 + 토큰)을 발급
2. 브라우저가 그 티켓으로 Storage 에 **직접** 업로드 (Vercel 함수 안 거침)
...
이 요청의 본문은 `["jpg", "png"]` 같은 **확장자 배열**뿐입니다. 몇 바이트짜리 JSON 이라 4.5MB 와는 영원히 무관해요.

여기서 두 가지를 미리 못 박는다는 점이 중요합니다.

- **`applicationId`** — 신청 한 건의 식별자를 _업로드 전에_ 정합니다. 사진 경로도, 나중에 들어갈 DB row 의 PK 도 모두 이 UUID 를 씁니다.
...
// 용도는 단 하나 — 서명 업로드 URL 로 사진을 Storage 에 직접 올리는 것.
// 서명 업로드는 토큰으로 인가되므로 anon 키로도 동작하고,
// 버킷에 RLS 가 켜져 있어 일반 조회/쓰기는 여전히 불가능하다.
export function supabaseBrowser(): SupabaseClient { /* ... */ }
```

anon 키가 노출되는 게 불안할 수 있는데, anon 키는 **원래 공개되어도 되는 값**입니다. 실제 인가는 서명 토큰이 하고, 버킷 RLS 때문에 토큰 없는 조회·쓰기는 막혀 있어요. 토큰은 우리 서버만 발급합니다.

업로드는 `uploadToSignedUrl` 한 줄입니다.
...
사진이 다 올라간 뒤, 신청서 텍스트 필드는 별도 JSON 으로 보냅니다. 이때 사진은 **파일이 아니라 경로 문자열**로만 전달돼요.
...
`/api/apply` 가 받는 본문은 이제 텍스트 + 경로 몇 줄. 사진이 아무리 커져도 이 JSON 은 늘 작습니다. **4.5MB 한도가 구조적으로 닿을 수 없는 곳**으로 옮긴 거예요.

## 직접 업로드가 새로 만든 빈틈 — 그리고 두 개의 자물쇠

여기서 멈추면 안 됩니다. 직접 업로드는 한도 문제를 풀었지만, 동시에 **빈틈 하나를 새로 만들었어요.**

수정 전에는 서버 함수가 사진 `File` 을 직접 손에 쥐고 있었습니다. 무엇을 어디에 올릴지 100% 서버가 알았어요. 수정 후에는 `/api/apply` 가 사진의 **경로 문자열을 클라이언트한테서 받습니다.** 클라이언트가 보내는 값은 — 당연히 — 신뢰할 수 없어요.
...
`p.startsWith(`${appId}/`)` 한 줄이 "남의 폴더 가리키기"를 막습니다. 검증의 기준이 되는 `appId` 는 같은 요청에 들어온 값이라, 결국 클라이언트가 만질 수 있는 건 **자기 자신의 폴더뿐**이에요. 그리고 그 폴더에 무언가를 올리려면 1단계에서 우리 서버가 발급한 서명 토큰이 있었어야 합니다. 사슬이 닫힙니다.

### 자물쇠 B — 실제로 존재하는지 확인

경로 형태가 맞다고 파일이 _진짜로_ 거기 있는 건 아닙니다. 그래서 insert 전에 Storage 를 실제로 조회해요.
...
이건 보안용이기도 하지만, 사실 **흔한 사고를 막는 안전망**에 더 가깝습니다. 2단계 업로드가 일부만 성공했는데 3단계가 그냥 진행되면, 사진 경로는 DB 에 적혔는데 실제 파일은 없는 신청서가 만들어져요. 운영자가 그걸 열었을 때 깨진 이미지를 보게 됩니다. 존재 확인 한 번이 그 상태를 입구에서 거릅니다.
...
"몇 배 빨라졌다"를 글에 적고 싶다면 추측이 아니라 측정해야 하는 이유가 이거예요. 측정 방법은 아래에 따로 정리했습니다.
...
그런데 사용자 입장에서 블러는 **기다릴 이유가 없는 작업**입니다. 신청이 접수됐는지만 알면 돼요. 블러본은 나중에 운영자가 운영 할 때나 필요하고요. 그래서 Next.js 15 의 `after()` 로 블러를 응답 _뒤로_ 뺐습니다.
...
그리고 이 이동에는 **숨은 비용**이 하나 있어요. 블러가 응답 안에 있을 때는 실패하면 그냥 사용자에게 에러를 주면 됐습니다. 응답 밖으로 빼는 순간 — 사용자는 이미 "접수 완료"를 받았는데 블러가 실패할 수 있어요. 그래서 `catch` 에서 접수를 실패시키지 않고 `admin_note` 에 실패를 기록합니다. **백그라운드로 옮긴 작업은 실패를 따로 감지하고 재처리할 길을 반드시 같이 만들어야 해요.** 그게 `after()` 의 진짜 설계 비용입니다.

(참고로 블러 강도 `PHOTO_BLUR_SIGMA` 는 sharp 의 blur sigma 값인데, 사진 해상도에 따라 체감이 달라져서 운영하며 12 → 15 → 25 로 올려가며 튜닝했습니다. 이건 코드 한 줄 상수라 글의 본론은 아니에요.)

## 측정 — 무엇을, 어떻게

> 이 절의 표는 의도적으로 비워뒀습니다. 정확한 숫자는 **배포 환경 + 실제 회선/기기**에서 측정해야 의미가 있어서, 측정 방법만 적어두고 값은 직접 채울 예정입니다.
...
- **같은 사진 세트** — 예: 기본 3장 + 옵션 1장, 장당 ~4MB 로 통일
- **회선 고정** — Chrome DevTools → Network → throttling 을 `Fast 4G` 같은 프리셋으로
- **여러 번 측정 후 중앙값** — 1회 측정은 노이즈가 큼. 최소 5회

순차 버전은 비교를 위해 잠깐 되돌려서 같은 조건으로 잽니다.

| 구간 (본인 3 + 전신 1, ~4MB/장) | 순차 (median) | 병렬 (median) | 단축 |
| ------------------------------- | ------------- | ------------- | ---- |
| Fast 4G                         |               |               |      |
| Wi-Fi                           |               |               |      |
...
또는 브라우저 DevTools → Network 패널에서 `/api/apply` 요청의 **TTFB** 를 봅니다. 비교를 위해 블러를 잠깐 `after()` 밖(응답 안)으로 되돌려 같은 측정을 한 번 더 해요.
...
두 가지 길이 있습니다. **운영 중인 값**을 보려면 `after()` 콜백 안의 `sharp` 파이프라인을 `console.time` 으로 감싸 Vercel 함수 로그에서 확인해요. **배포 전에 미리 가늠**하려면 저장소의 `scripts/bench-blur.mjs` 를 실행합니다.
...
도구를 한 줄로 정리하면 — `performance.now()` / `Date.now()` 로깅, DevTools Network 스로틀 + TTFB, Vercel 함수 로그. 더 깔끔하게 보고 싶으면 응답에 `Server-Timing` 헤더를 실어 Network 패널에서 바로 읽는 방법도 있습니다.

## 일반화 — 다음에 또 마주칠 함정

이번 작업을 한 줄로 줄이면 — **큰 페이로드를 서버 함수의 경로에서 빼라.** 그리고 뺄 때 따라오는 두 가지를 같이 챙겨야 합니다.

1. **신뢰 경계가 이동한다.** 파일을 직접 쥐던 서버가, 파일에 *대한 주장*을 받는 서버가 됩니다. 경로·존재 여부를 서버가 다시 검증하세요.
2. **백그라운드로 옮긴 작업은 실패가 조용해진다.** 사용자는 이미 성공 응답을 받았습니다. 실패를 감지하고 재처리할 길(`admin_note`, 잡 큐, 재시도 등)을 반드시 같이 깔아두세요.
...
마지막 줄이 특히 빠지기 쉽고, 빠지면 "DB 엔 멀쩡한데 실물이 없는" 종류의 버그가 됩니다.
