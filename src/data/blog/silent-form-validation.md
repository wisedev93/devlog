...
이걸로 silent failure가 완전히 해결되는 건 아닙니다. 어떤 항목이 문제인지는 여전히 사용자가 찾아야 해요. 그래도 버튼이 죽은 것처럼 보이는 최악의 상태는 막습니다. 검증이 실패했다는 사실을 알리는 게 1차 목표고, 정확한 위치를 짚어주는 건 그다음이라고 봤습니다.
