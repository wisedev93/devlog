...
이 글은 거기에 Sentry를 붙인 기록입니다. 무엇을 어떻게 붙였는지, 왜 그렇게 결정했는지, 그리고 그 대가로 받은 번들 사이즈 청구서가 얼마였는지. 500KB도 안 되던 번들이 모니터링 한 번에 20% 넘게 불어난 게 감수할 만한 비용이었는지까지 따라갑니다.

## Table of contents

## 무엇이 없었나

문제를 요약하면, 에러가 사용자의 입을 통해서만 우리에게 도착했다는 것입니다.
...
번들 청구서는 gzip +97KB(+20.7%), 그중 +89KB가 메인 엔트리 청크였습니다. 원인은 Replay(rrweb, 단독 41KB)와 그걸 진입점에서 정적으로 import한 구조였어요. B2B 어드민 특성상 감수할 만한 트레이드오프라고 판단해 Replay를 유지하되, 별도 모듈로 만들어 lazy-load했습니다. 엔트리 청크는 267에서 227KB로 40KB 줄었고, 총 바이트는 그대로입니다. 없애는 게 아니라 옮기는 것이니까요.
